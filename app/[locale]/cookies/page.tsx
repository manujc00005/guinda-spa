...
export const metadata = {
  title: `${LEGAL_TEXTS.cookies.title} | ${COMPANY_DATA.brandName}`,
  description: `${LEGAL_TEXTS.cookies.title} de ${COMPANY_DATA.brandName}. Información sobre el uso de cookies en nuestro sitio web.`,
    ...
    <div className="min-h-screen bg-white pt-28 pb-20">
      <div className="container-page max-w-4xl">
        ...
        <nav className="mb-8 text-sm">
          ...
          <span className="text-(--color-text-primary) font-medium">Política de Cookies</span>
        </nav>

        {/* Header */}
        <header className="mb-12">
          <h1 className="text-4xl md:text-5xl font-playfair font-semibold text-(--color-text-primary) mb-4">
            {LEGAL_TEXTS.cookies.title}
          </h1>
          <p className="text-sm text-(--color-text-secondary)">{LEGAL_TEXTS.cookies.lastUpdated}</p>
        </header>

        {/* Content */}
        <div className="prose prose-lg max-w-none">
          {LEGAL_TEXTS.cookies.sections.map((section, index) => (
