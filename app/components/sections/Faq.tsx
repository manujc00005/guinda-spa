...
/**
 * SECCIÓN FAQ CON CTA WHATSAPP — i18n
 *
 * Todas las preguntas y textos vienen de traducciones.
 */
        ...
        <div className="mb-12 p-8 md:p-10 bg-(--color-ivory) rounded-2xl border border-(--color-border)">
          <div className="text-center">
            ...
            <p className="text-xs text-(--color-text-secondary)">
              {t("contact.hoursPrefix")} {tCompany("businessHours.weekdays")}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          {FAQ_KEYS.map((key) => (
            <Accordion
              key={key}
              pregunta={{
                pregunta: t(`questions.${key}.question`),
                respuesta: t(`questions.${key}.answer`),
              }}
            />
          ))}
        </div>
